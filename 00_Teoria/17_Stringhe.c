...
Si accede a una stringa mediante un puntatore al suo primo carattere. Il valore di una stringa è l0indirizzo del suo primo carattere.
Da qui ricaviamo che una stringa non è altro che un array di caratteri.
Ogni stringa finisce con "/0", il carattere nullo. Se non c'è spazio per questo carattere, è molto possibile che il codice vada in errore
